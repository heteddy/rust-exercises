...
pub async fn start_cacher() -> mpsc::Sender<sync::SyncData> {
    let (entity_tx, entity_rx) = mpsc::channel::<sync::SyncData>(1);
    // 这里使用了锁，因此内部就不需要再上锁了
    let conf_repo: Arc<std::sync::RwLock<repo::IndexConfigRepo>> =
        repo::IndexConfigRepo::get_instance(); // 并非全局变量需要用arc
                                               // 这里不能用lazy static，类型不一样，也不想解引用;
                                               // 是否可以用once cell，然后get获取arc并clone应该也可以实现
    {

        let mut repo = conf_repo.write().unwrap();
        repo.init().await;
    }
    ...
    let (sync_tx, sync_rx) = mpsc::channel::<sync::SyncData>(1);
